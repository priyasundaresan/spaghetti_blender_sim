...
RED_RECEPTACLE_LOC = (6,0,-0.25)
BLUE_RECEPTACLE_LOC = (-6,0,-0.25)
...
def make_walls(params):
    ...
    bpy.ops.mesh.primitive_plane_add(size=1, location=(0,dim/2,3), rotation=(np.pi/2,0,0))
...
def get_reward_stats(items, colors):
    ...
    for item, color in zip(items,colors):
        dist_to_red_receptacle = np.linalg.norm(np.array(item.matrix_world.translation[:2]) - RED_RECEPTACLE_LOC[:2])
        dist_to_blue_receptacle = np.linalg.norm(np.array(item.matrix_world.translation[:2]) - BLUE_RECEPTACLE_LOC[:2])
