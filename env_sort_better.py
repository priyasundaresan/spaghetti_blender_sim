...
class BlockSortEnv(Env):
    ...
    def step(self, action):
        ...
        print('\naction %d: %s, '%(self.action_ctr, self.get_action_meanings()[int(action)]), 'reward: %f, '%reward, \
                                   '\nred_correct: %d'%red_correct, \
                                   'blue_correct: %d'%blue_correct, \
                                   'red_incorrect: %d'%red_incorrect, \
                                   'blue_incorrect: %d'%blue_incorrect, \
                                   'gain: %d'%(correct_reward), 'num correct: %d'%(red_correct + blue_correct))

        #done = (self.action_ctr >= self.max_action_count) or (pick_item is None)
        done = (self.action_ctr >= self.max_action_count) or (red_correct + blue_correct == len(self.items) or (pick_item is None))
        if done:
            clear_items()
        ...
        return obs, reward, done, (action_pixels, red_correct + blue_correct, red_incorrect + blue_incorrect)
